...
const fitJapaneseHeadings=()=>{
  document.querySelectorAll('.fit-heading').forEach(heading=>{
    ...
    const minSize=Math.max(28,computed*.72);
    let size=computed;
    const available=heading.clientWidth;
    while(size>minSize&&phrases.some(phrase=>phrase.scrollWidth>available)){
      size-=.5;
